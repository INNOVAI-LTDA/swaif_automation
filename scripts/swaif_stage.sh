...
ai_generate() {
  ...
  python3 scripts/swaif_ai_generate.py "${target}" "${prompt}"
}
...
case "${stage}" in
  init)
    ...
    ;;
  specify)
    ai_generate "${spec}" "Escreva a SPEC para '${feature_slug}' usando ${intake}. Foque no WHAT/WHY, sem stack. Inclua critérios de aceite e não-objetivos."
    ;;
  plan)
    [[ -f "${spec}" ]] || { echo "Pré-requisito ausente: ${spec}"; exit 1; }
    ai_generate "${plan}" "Crie um plano técnico para '${feature_slug}' com Python/FastAPI/Postgres, baseado em ${spec}. Endpoints, modelos e testes."
    ;;
  tasks)
    [[ -f "${plan}" ]] || { echo "Pré-requisito ausente: ${plan}"; exit 1; }
    ai_generate "${tasks}" "Gere tasks pequenas (<=2h) com critério de aceite, baseado em ${plan}."
    ;;
  implement)
